const beers = [
  ...
  //6
  let nombreIngresado = prompt("Ingrese el nombre de una cerveza");
  const nombres = beers.find(beers => beers.name == nombreIngresado);
  console.log(nombres);
  
  
  //7
  let ibuIngresado = Number(prompt("Ingrese el IBU de una cerveza"));
  const valorIbu = beers.find(beers => beers.ibu === ibuIngresado);
  console.log(valorIbu);
  
  if (valorIbu === undefined) {
    console.log("No existe una cerveza con un IBU de " + ibuIngresado);
  }
  
  
  //8
  function posicion( nombreIngresado ){
      const posicion = beers.findIndex(beers => beers.name == nombreIngresado);
      console.log(posicion);
      if(nombres === undefined) {
        console.log(`${nombreIngresado} no existe`)
      }
  }
  
  posicion(nombreIngresado)
  
  
  //9
  function noExcede( beers ){
      const resultado = beers.filter( beers => beers.abv < 8 )
                          .map (beers => {
                            return {
                              name: beers.name,
                              abv: beers.abv,
  ...
  /*
  //10
  let property = prompt("Ingresa una propiedad");
  function trueOFalse( beers, property ){
      const resultado = beers.filter(beers => beers[property]) 
           .toSorted((a, b)=> {
            if(property === true) return (a[property] - b[property])
            else if(property === false) return (b[property] - a[property])
          }).slice(0, 10)
          console.log( resultado )
  }
  
  trueOFalse(beers, property)
  */
  //11
  ...
  function renderizar(beers, table){
    for (const beer of beers) {
      const tabla = crearTabla(beer);
      table.appendChild(tabla);
    }
  }
    
  renderizar(beers, divTable)